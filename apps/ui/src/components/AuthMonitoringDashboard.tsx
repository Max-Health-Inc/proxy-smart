...
export function AuthMonitoringDashboard({ embedded, isRealTimeActive: parentRealTime }: AuthMonitoringDashboardProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [eventsData, analyticsData] = await Promise.all([
        authMonitoringService.fetchEvents({ limit: 200 }),
        authMonitoringService.fetchAnalytics(),
      ]);
      setEvents(eventsData.events || []);
      setAnalytics(analyticsData);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load auth events data');
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const init = async () => {
      await fetchData();
      try {
        await authMonitoringService.connect();
        eventUnsub = authMonitoringService.onEvent((event) => {
          if (isRealTimeActiveRef.current) {
            setEvents(prev => [event, ...prev.slice(0, 999)]);
          }
        });
        analyticsUnsub = authMonitoringService.onAnalytics((a) => {
          if (isRealTimeActiveRef.current) {
            setAnalytics(a);
          }
        });
      } catch (err) {
        console.error('Auth monitoring SSE connection failed:', err);
      }
    };
    ...
    return () => {
      eventUnsub?.();
      analyticsUnsub?.();
      authMonitoringService.disconnect();
    };
  }, [fetchData]);
  ...
  if (isLoading) return <PageLoadingState message={t('Loading auth events data...')} />;
    ...
    <div className="space-y-6">
      ...
      {!embedded && (
        <RealTimeBanner
          ...
          activeTitle={t('Real-time Auth Monitoring Active')}
          pausedTitle={t('Real-time Auth Monitoring Paused')}
          activeDescription={t('Authentication events are pushed in real time as they occur.')}
        ...
        <div className={embedded ? '' : 'p-8 pb-6'}>
          {!embedded && (
            <div className="flex items-center space-x-3 mb-6">
              ...
              <div>
                <h3 className="text-xl font-bold text-foreground tracking-tight">{t('Auth Events Dashboard')}</h3>
                <p className="text-muted-foreground font-medium">{t('Track logins, logouts, registrations, and token exchanges')}</p>
              </div>
            </div>
          )}
          ...
          <Tabs defaultValue="overview" className="space-y-6">
            ...
            <TabsContent value="overview" className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                ...
                <StatCard icon={CheckCircle} label={t('Success Rate')} value={`${(analytics?.successRate ?? 0).toFixed(1)}%`} subtitle={t('Successful auth events')} color="green" />
                <StatCard icon={LogIn} label={t('Login Events')} value={((analytics?.eventsByType?.['LOGIN'] ?? 0) + (analytics?.eventsByType?.['LOGIN_ERROR'] ?? 0)).toLocaleString()} subtitle={t('Logins + errors')} color="blue" />
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-card/70 backdrop-blur-sm p-6 rounded-2xl border border-border/50 shadow-lg">
                  <div className="flex items-center space-x-3 mb-6">
                    ...
                    <div>
                      <h4 className="text-lg font-bold text-foreground tracking-tight">{t('Auth Activity (24h)')}</h4>
                ...
                <div className="bg-card/70 backdrop-blur-sm p-6 rounded-2xl border border-border/50 shadow-lg">
                  <div className="flex items-center space-x-3 mb-6">
                    ...
                    <div>
                      <h4 className="text-lg font-bold text-foreground tracking-tight">{t('Event Types')}</h4>
                      <p className="text-muted-foreground font-medium">{t('Distribution by event type')}</p>
                    </div>
                  </div>
                  <div className="h-[300px]">
              ...
              <div className="bg-card/70 backdrop-blur-sm rounded-2xl border border-border/50 shadow-lg overflow-hidden">
                <div className="p-6">
                  <div className="flex items-center space-x-3 mb-4">
                    ...
                    <div>
                      <h4 className="text-lg font-bold text-foreground">{t('Recent Auth Failures')}</h4>
                      <p className="text-muted-foreground text-sm">{t('Failed logins, expired tokens, rejected registrations')}</p>
                    </div>
                  </div>
                </div>
