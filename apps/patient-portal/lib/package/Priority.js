...
export async function validatePriority(resource, options) {
    const errors = [];
    const warnings = [];
    ...
    const result2 = await fhirpath.evaluate(resource, "valueCodeableConcept.all(hasValue() or (children().count() > id.count()))", { resource }, fhirpath_model, fhirpathOptions);
    ...
    const result5 = await fhirpath.evaluate(resource, "valueCodeableConcept.exists()", { resource }, fhirpath_model, fhirpathOptions);
    if (!result5.every(Boolean)) {
        errors.push("Constraint violation: valueCodeableConcept must be present");
    }
    ...
    const result7 = await fhirpath.evaluate(resource, "extension.count() <= 0", { resource }, fhirpath_model, fhirpathOptions);
    if (!result7.every(Boolean)) {
        errors.push("Constraint violation: extension must contain at most 0 elements");
    }
    const result8 = await fhirpath.evaluate(resource, "extension.exists().not()", { resource }, fhirpath_model, fhirpathOptions);
    ...
    if (resource.url !== undefined && resource.url !== "http://hl7.org/fhir/StructureDefinition/flag-priority") {
        errors.push("url must have the fixed value 'http://hl7.org/fhir/StructureDefinition/flag-priority'");
    }
    ...
    // Choice-type narrowing: value[x] allows only: CodeableConcept
    {
        const _ckAllowed = ['valueCodeableConcept'];
        for (const _ckKey of Object.keys(_bRes)) {
            if (_ckKey.startsWith('value') && _ckKey.length > 5 && _ckKey.charCodeAt(5) >= 65 && _ckKey.charCodeAt(5) <= 90 && !_ckAllowed.includes(_ckKey) && _bRes[_ckKey] !== undefined) {
                errors.push("The Profile 'http://hl7.org/fhir/StructureDefinition/flag-priority' definition allows for the type CodeableConcept but found type " + _ckKey.substring(5));
            }
        }
    }
