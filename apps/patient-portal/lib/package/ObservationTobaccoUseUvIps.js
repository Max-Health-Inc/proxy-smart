...
export async function validateObservationTobaccoUseUvIps(resource, options) {
    const errors = [];
    const warnings = [];
    ...
    // Pattern constraint for code: must include coding with system="http://loinc.org" and code="72166-2"
    if (resource.code) {
        const code_pattern0Valid = resource.code.coding?.some((coding) => coding?.system === "http://loinc.org" && coding?.code === "72166-2");
        if (!code_pattern0Valid) {
            errors.push("code must include a coding with system 'http://loinc.org' and code '72166-2'");
        }
    }
    ...
    // Choice-type narrowing: effective[x] allows only: dateTime
    {
        ...
        for (const _ckKey of Object.keys(_bRes)) {
            if (_ckKey.startsWith('effective') && _ckKey.length > 9 && _ckKey.charCodeAt(9) >= 65 && _ckKey.charCodeAt(9) <= 90 && !_ckAllowed.includes(_ckKey) && _bRes[_ckKey] !== undefined) {
                errors.push("The Profile 'http://hl7.org/fhir/uv/ips/StructureDefinition/Observation-tobaccouse-uv-ips' definition allows for the type dateTime but found type " + _ckKey.substring(9));
            }
        }
    }
    // Choice-type narrowing: value[x] allows only: Quantity, CodeableConcept, string, boolean, integer, Range, Ratio, SampledData, time, dateTime, Period
    {
        ...
        for (const _ckKey of Object.keys(_bRes)) {
            if (_ckKey.startsWith('value') && _ckKey.length > 5 && _ckKey.charCodeAt(5) >= 65 && _ckKey.charCodeAt(5) <= 90 && !_ckAllowed.includes(_ckKey) && _bRes[_ckKey] !== undefined) {
                errors.push("The Profile 'http://hl7.org/fhir/uv/ips/StructureDefinition/Observation-tobaccouse-uv-ips' definition allows for the type Quantity, CodeableConcept, string, boolean, integer, Range, Ratio, SampledData, time, dateTime, Period but found type " + _ckKey.substring(5));
            }
        }
    }
