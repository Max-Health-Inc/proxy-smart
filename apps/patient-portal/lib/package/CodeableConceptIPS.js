...
import { validateCodingIPS } from './CodingIPS.js';
export async function validateCodeableConceptIPS(resource, options) {
    const errors = [];
    const warnings = [];
    ...
    const result0 = await fhirpath.evaluate(resource, "CodeableConcept.all(hasValue() or (children().count() > id.count()))", { resource }, fhirpath_model, fhirpathOptions);
    ...
    const result2 = await fhirpath.evaluate(resource, "coding.all(hasValue() or (children().count() > id.count()))", { resource }, fhirpath_model, fhirpathOptions);
    if (!result2.every(Boolean)) {
        errors.push("Constraint violation: All FHIR elements must have a @value or children");
    }
    const result3 = await fhirpath.evaluate(resource, "text.all(hasValue() or (children().count() > id.count()))", { resource }, fhirpath_model, fhirpathOptions);
    ...
    // Structural type check: coding must be a JSON Array
    if (_bRes.coding !== undefined && _bRes.coding !== null && !Array.isArray(_bRes.coding)) {
        errors.push("The property coding must be a JSON Array, not an Object (at " + (_bRes.resourceType || "Resource") + ")");
    }
    // Structural type check: extension must be a JSON Array
    if (_bRes.extension !== undefined && _bRes.extension !== null && !Array.isArray(_bRes.extension)) {
        errors.push("The property extension must be a JSON Array, not an Object (at " + (_bRes.resourceType || "Resource") + ")");
    }
    // Profiled datatype delegation: coding → CodingIPS
    if (resource.coding && Array.isArray(resource.coding)) {
        for (const _dtEl of resource.coding) {
            const _sub = await validateCodingIPS(_dtEl, options);
