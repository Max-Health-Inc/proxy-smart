...
export async function validateNote(resource, options) {
    const errors = [];
    const warnings = [];
    ...
    const result2 = await fhirpath.evaluate(resource, "valueAnnotation.all(hasValue() or (children().count() > id.count()))", { resource }, fhirpath_model, fhirpathOptions);
    ...
    const result6 = await fhirpath.evaluate(resource, "valueAnnotation.exists()", { resource }, fhirpath_model, fhirpathOptions);
    if (!result6.every(Boolean)) {
        errors.push("Constraint violation: valueAnnotation must be present");
    }
    ...
    if (resource.url !== undefined && resource.url !== "http://hl7.org/fhir/StructureDefinition/note") {
        errors.push("url must have the fixed value 'http://hl7.org/fhir/StructureDefinition/note'");
    }
    ...
    // Choice-type narrowing: value[x] allows only: Annotation
    {
        const _ckAllowed = ['valueAnnotation'];
        for (const _ckKey of Object.keys(_bRes)) {
            if (_ckKey.startsWith('value') && _ckKey.length > 5 && _ckKey.charCodeAt(5) >= 65 && _ckKey.charCodeAt(5) <= 90 && !_ckAllowed.includes(_ckKey) && _bRes[_ckKey] !== undefined) {
                errors.push("The Profile 'http://hl7.org/fhir/StructureDefinition/note' definition allows for the type Annotation but found type " + _ckKey.substring(5));
            }
        }
    }
