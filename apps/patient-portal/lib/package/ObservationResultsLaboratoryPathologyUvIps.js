...
export async function validateObservationResultsLaboratoryPathologyUvIps(resource, options) {
    const errors = [];
    const warnings = [];
    ...
    const result22 = await fhirpath.evaluate(resource, "effective.all(hasValue() or (children().count() > id.count()))", { resource }, fhirpath_model, fhirpathOptions);
    ...
    const result43 = await fhirpath.evaluate(resource, "effectiveDateTime.all(hasValue() or (children().count() > id.count()))", { resource }, fhirpath_model, fhirpathOptions);
    if (!result43.every(Boolean)) {
        errors.push("Constraint violation: All FHIR elements must have a @value or children");
    }
    const result44 = await fhirpath.evaluate(resource, "valueString.all(hasValue() or (children().count() > id.count()))", { resource }, fhirpath_model, fhirpathOptions);
    if (!result44.every(Boolean)) {
        errors.push("Constraint violation: All FHIR elements must have a @value or children");
    }
    const result45 = await fhirpath.evaluate(resource, "valueQuantity.all(hasValue() or (children().count() > id.count()))", { resource }, fhirpath_model, fhirpathOptions);
    if (!result45.every(Boolean)) {
        errors.push("Constraint violation: All FHIR elements must have a @value or children");
    }
    const result46 = await fhirpath.evaluate(resource, "valueCodeableConcept.all(hasValue() or (children().count() > id.count()))", { resource }, fhirpath_model, fhirpathOptions);
    ...
    const result48 = await fhirpath.evaluate(resource, "category.count() >= 1", { resource }, fhirpath_model, fhirpathOptions);
    if (!result48.every(Boolean)) {
        errors.push("Constraint violation: category must contain at least one element");
    }
    const result49 = await fhirpath.evaluate(resource, "code.exists()", { resource }, fhirpath_model, fhirpathOptions);
    if (!result49.every(Boolean)) {
        errors.push("Constraint violation: code must be present");
    }
    const result50 = await fhirpath.evaluate(resource, "subject.exists()", { resource }, fhirpath_model, fhirpathOptions);
    if (!result50.every(Boolean)) {
        errors.push("Constraint violation: subject must be present");
    }
    const result51 = await fhirpath.evaluate(resource, "effective.exists()", { resource }, fhirpath_model, fhirpathOptions);
    if (!result51.every(Boolean)) {
        errors.push("Constraint violation: effective must be present");
    }
    const result52 = await fhirpath.evaluate(resource, "performer.count() >= 1", { resource }, fhirpath_model, fhirpathOptions);
    if (!result52.every(Boolean)) {
        errors.push("Constraint violation: performer must contain at least one element");
    }
    // Pattern constraint for category: at least one element must include coding with system="http://terminology.hl7.org/CodeSystem/observation-category" and code="laboratory"
    if (resource.category && Array.isArray(resource.category) && resource.category.length > 0) {
        const category_pattern0Valid = resource.category.some((item) => item?.coding?.some((coding) => coding?.system === "http://terminology.hl7.org/CodeSystem/observation-category" && coding?.code === "laboratory"));
        if (!category_pattern0Valid) {
            errors.push("category must include at least one element with a coding with system 'http://terminology.hl7.org/CodeSystem/observation-category' and code 'laboratory'");
        }
    }
    ...
    // Choice-type narrowing: effective[x] allows only: dateTime, Period
    {
        ...
        for (const _ckKey of Object.keys(_bRes)) {
            if (_ckKey.startsWith('effective') && _ckKey.length > 9 && _ckKey.charCodeAt(9) >= 65 && _ckKey.charCodeAt(9) <= 90 && !_ckAllowed.includes(_ckKey) && _bRes[_ckKey] !== undefined) {
                errors.push("The Profile 'http://hl7.org/fhir/uv/ips/StructureDefinition/Observation-results-laboratory-pathology-uv-ips' definition allows for the type dateTime, Period but found type " + _ckKey.substring(9));
            }
        }
    }
    // Choice-type narrowing: value[x] allows only: Quantity, CodeableConcept, string, boolean, integer, Range, Ratio, SampledData, time, dateTime, Period
    {
        ...
        for (const _ckKey of Object.keys(_bRes)) {
            if (_ckKey.startsWith('value') && _ckKey.length > 5 && _ckKey.charCodeAt(5) >= 65 && _ckKey.charCodeAt(5) <= 90 && !_ckAllowed.includes(_ckKey) && _bRes[_ckKey] !== undefined) {
                errors.push("The Profile 'http://hl7.org/fhir/uv/ips/StructureDefinition/Observation-results-laboratory-pathology-uv-ips' definition allows for the type Quantity, CodeableConcept, string, boolean, integer, Range, Ratio, SampledData, time, dateTime, Period but found type " + _ckKey.substring(5));
            }
        }
    }
    // Required slice validation for category:laboratory
    if (resource.category && Array.isArray(resource.category)) {
        const laboratoryElements = resource.category.filter((item) => item.coding?.some((c) => c.system === "http://terminology.hl7.org/CodeSystem/observation-category" && c.code === "laboratory"));
        if (laboratoryElements.length < 1) {
            errors.push("No elements matched required slice: 'category:laboratory'");
        }
    }
    else {
        errors.push("No elements matched required slice: 'category:laboratory'");
