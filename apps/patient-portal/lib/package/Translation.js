...
export async function validateTranslation(resource, options) {
    const errors = [];
    const warnings = [];
    ...
    const result6 = await fhirpath.evaluate(resource, "value.exists().not()", { resource }, fhirpath_model, fhirpathOptions);
    if (!result6.every(Boolean)) {
        errors.push("Constraint violation: value: max allowed = 0, but found 1");
    }
    // Fixed value constraint for url
    if (resource.url !== undefined && resource.url !== "http://hl7.org/fhir/StructureDefinition/translation") {
        errors.push("url must have the fixed value 'http://hl7.org/fhir/StructureDefinition/translation'");
    }
    ...
    // Required extension slice validation for extension:lang (url discriminator)
    if (resource.extension && Array.isArray(resource.extension)) {
        const langElements = resource.extension.filter((item) => item.url === "lang");
        if (langElements.length < 1) {
            errors.push("Slice 'extension:lang': a matching slice is required, but not found");
        }
    }
    else {
        errors.push("Slice 'extension:lang': a matching slice is required, but not found");
    }
    // Required extension slice validation for extension:content (url discriminator)
    if (resource.extension && Array.isArray(resource.extension)) {
        const contentElements = resource.extension.filter((item) => item.url === "content");
        if (contentElements.length < 1) {
            errors.push("Slice 'extension:content': a matching slice is required, but not found");
        }
    }
    else {
        errors.push("Slice 'extension:content': a matching slice is required, but not found");
