import { validateObservationAlcoholUseUvIps } from "./ObservationAlcoholUseUvIps.js";
...
export class ObservationAlcoholUseUvIpsClass {
    constructor(resource) {
        this.resource = resource;
        ensureProfile(this.resource, 'http://hl7.org/fhir/uv/ips/StructureDefinition/Observation-alcoholuse-uv-ips');
    }
    /** Validate current resource instance. */
    async validate(options) {
        return validateObservationAlcoholUseUvIps(this.resource, options);
    }
    getResource() {
    ...
    /**
     * Create a minimal randomized instance of ObservationAlcoholUseUvIps.
    ...
    static random(overrides = {}, opts) {
        ...
        const base = { resourceType: 'Observation', id: randomId(), status: "final", code: skeletonCodeableConcept('http://loinc.org', ['74013-4']), subject: { reference: 'Patient/' + randomId() }, effectiveDateTime: randomDate(), };
        // Always include meta.profile with this profile's canonical URL if available & only for resource profiles
        ensureProfile(base, 'http://hl7.org/fhir/uv/ips/StructureDefinition/Observation-alcoholuse-uv-ips');
        // Enrich the base resource with common fields using centralized enrichment logic
        enrichResource(base, 'Observation', 'http://hl7.org/fhir/uv/ips/StructureDefinition/Observation-alcoholuse-uv-ips', ObservationAlcoholUseUvIpsClass._fieldPatterns, ObservationAlcoholUseUvIpsClass._forbiddenFields, ObservationAlcoholUseUvIpsClass.valueSetBindings, ObservationAlcoholUseUvIpsClass._codeResolver, ObservationAlcoholUseUvIpsClass._fieldOrder);
        // Cast through unknown to acknowledge dynamic enrichment
        return { ...base, ...overrides };
    }
    /** Convenience: returns a class instance wrapping a randomized resource. */
    static randomClass(overrides = {}, opts) {
        return new ObservationAlcoholUseUvIpsClass(this.random(overrides, opts));
    }
}
/** Pattern constraints for profile fields (used by random() generator) */
ObservationAlcoholUseUvIpsClass._fieldPatterns = {
    "code": {
        "coding": [
            {
...
ObservationAlcoholUseUvIpsClass._forbiddenFields = ["component"];
...
ObservationAlcoholUseUvIpsClass.valueSetBindings = {
...
ObservationAlcoholUseUvIpsClass._codeResolver = (() => {
