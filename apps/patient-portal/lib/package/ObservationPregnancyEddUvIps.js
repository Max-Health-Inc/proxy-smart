...
export async function validateObservationPregnancyEddUvIps(resource, options) {
    const errors = [];
    const warnings = [];
    ...
    const result43 = await fhirpath.evaluate(resource, "valueDateTime.all(hasValue() or (children().count() > id.count()))", { resource }, fhirpath_model, fhirpathOptions);
    ...
    const result51 = await fhirpath.evaluate(resource, "method.exists().not()", { resource }, fhirpath_model, fhirpathOptions);
    if (!result51.every(Boolean)) {
        errors.push("Constraint violation: method: max allowed = 0, but found 1");
    }
    const result52 = await fhirpath.evaluate(resource, "specimen.exists().not()", { resource }, fhirpath_model, fhirpathOptions);
    if (!result52.every(Boolean)) {
        errors.push("Constraint violation: specimen: max allowed = 0, but found 1");
    }
    const result53 = await fhirpath.evaluate(resource, "device.exists().not()", { resource }, fhirpath_model, fhirpathOptions);
    if (!result53.every(Boolean)) {
        errors.push("Constraint violation: device: max allowed = 0, but found 1");
    }
    const result54 = await fhirpath.evaluate(resource, "referenceRange.exists().not()", { resource }, fhirpath_model, fhirpathOptions);
    if (!result54.every(Boolean)) {
        errors.push("Constraint violation: referenceRange: max allowed = 0, but found 1");
    }
    const result55 = await fhirpath.evaluate(resource, "component.exists().not()", { resource }, fhirpath_model, fhirpathOptions);
    ...
    // Prohibited field: Observation.method (max=0)
    if (_bRes.method !== undefined) {
        errors.push("Observation.method: max allowed = 0, but found 1");
    }
    ...
    // Choice-type narrowing: effective[x] allows only: dateTime
    {
        ...
        for (const _ckKey of Object.keys(_bRes)) {
            if (_ckKey.startsWith('effective') && _ckKey.length > 9 && _ckKey.charCodeAt(9) >= 65 && _ckKey.charCodeAt(9) <= 90 && !_ckAllowed.includes(_ckKey) && _bRes[_ckKey] !== undefined) {
                errors.push("The Profile 'http://hl7.org/fhir/uv/ips/StructureDefinition/Observation-pregnancy-edd-uv-ips' definition allows for the type dateTime but found type " + _ckKey.substring(9));
            }
        }
    }
    // Choice-type narrowing: value[x] allows only: Quantity, CodeableConcept, string, boolean, integer, Range, Ratio, SampledData, time, dateTime, Period
    {
        ...
        for (const _ckKey of Object.keys(_bRes)) {
            if (_ckKey.startsWith('value') && _ckKey.length > 5 && _ckKey.charCodeAt(5) >= 65 && _ckKey.charCodeAt(5) <= 90 && !_ckAllowed.includes(_ckKey) && _bRes[_ckKey] !== undefined) {
                errors.push("The Profile 'http://hl7.org/fhir/uv/ips/StructureDefinition/Observation-pregnancy-edd-uv-ips' definition allows for the type Quantity, CodeableConcept, string, boolean, integer, Range, Ratio, SampledData, time, dateTime, Period but found type " + _ckKey.substring(5));
            }
        }
    }
