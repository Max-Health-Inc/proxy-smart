...
export async function validateObservationPregnancyStatusUvIps(resource, options) {
    const errors = [];
    const warnings = [];
    ...
    const result43 = await fhirpath.evaluate(resource, "valueCodeableConcept.all(hasValue() or (children().count() > id.count()))", { resource }, fhirpath_model, fhirpathOptions);
    ...
    const result48 = await fhirpath.evaluate(resource, "referenceRange.count() <= 0", { resource }, fhirpath_model, fhirpathOptions);
    if (!result48.every(Boolean)) {
        errors.push("Constraint violation: referenceRange must contain at most 0 elements");
    }
    const result49 = await fhirpath.evaluate(resource, "component.count() <= 0", { resource }, fhirpath_model, fhirpathOptions);
    if (!result49.every(Boolean)) {
        errors.push("Constraint violation: component must contain at most 0 elements");
    }
    const result50 = await fhirpath.evaluate(resource, "bodySite.exists().not()", { resource }, fhirpath_model, fhirpathOptions);
    if (!result50.every(Boolean)) {
        errors.push("Constraint violation: bodySite: max allowed = 0, but found 1");
    }
    const result51 = await fhirpath.evaluate(resource, "specimen.exists().not()", { resource }, fhirpath_model, fhirpathOptions);
    if (!result51.every(Boolean)) {
        errors.push("Constraint violation: specimen: max allowed = 0, but found 1");
    }
    const result52 = await fhirpath.evaluate(resource, "device.exists().not()", { resource }, fhirpath_model, fhirpathOptions);
    if (!result52.every(Boolean)) {
        errors.push("Constraint violation: device: max allowed = 0, but found 1");
    }
    const result53 = await fhirpath.evaluate(resource, "referenceRange.exists().not()", { resource }, fhirpath_model, fhirpathOptions);
    if (!result53.every(Boolean)) {
        errors.push("Constraint violation: referenceRange: max allowed = 0, but found 1");
    }
    const result54 = await fhirpath.evaluate(resource, "component.exists().not()", { resource }, fhirpath_model, fhirpathOptions);
    if (!result54.every(Boolean)) {
        errors.push("Constraint violation: component: max allowed = 0, but found 1");
    }
    // Pattern constraint for code: must include coding with system="http://loinc.org" and code="82810-3"
    if (resource.code) {
        const code_pattern0Valid = resource.code.coding?.some((coding) => coding?.system === "http://loinc.org" && coding?.code === "82810-3");
        if (!code_pattern0Valid) {
            errors.push("code must include a coding with system 'http://loinc.org' and code '82810-3'");
        }
    }
    ...
    // Nested required field: hasMember.reference (min=1)
    if (resource.hasMember && Array.isArray(resource.hasMember)) {
        for (const _el of resource.hasMember) {
    ...
    // Prohibited field: Observation.bodySite (max=0)
    if (_bRes.bodySite !== undefined) {
        errors.push("Observation.bodySite: max allowed = 0, but found 1");
    }
    // Prohibited field: Observation.specimen (max=0)
    if (_bRes.specimen !== undefined) {
        errors.push("Observation.specimen: max allowed = 0, but found 1");
    }
    // Prohibited field: Observation.device (max=0)
    if (_bRes.device !== undefined) {
        errors.push("Observation.device: max allowed = 0, but found 1");
    }
    // Prohibited field: Observation.referenceRange (max=0)
    if (_bRes.referenceRange !== undefined) {
        errors.push("Observation.referenceRange: max allowed = 0, but found 1");
    }
    ...
    // Choice-type narrowing: effective[x] allows only: dateTime
    {
        ...
        for (const _ckKey of Object.keys(_bRes)) {
            if (_ckKey.startsWith('effective') && _ckKey.length > 9 && _ckKey.charCodeAt(9) >= 65 && _ckKey.charCodeAt(9) <= 90 && !_ckAllowed.includes(_ckKey) && _bRes[_ckKey] !== undefined) {
                errors.push("The Profile 'http://hl7.org/fhir/uv/ips/StructureDefinition/Observation-pregnancy-status-uv-ips' definition allows for the type dateTime but found type " + _ckKey.substring(9));
            }
        }
    }
    // Choice-type narrowing: value[x] allows only: Quantity, CodeableConcept, string, boolean, integer, Range, Ratio, SampledData, time, dateTime, Period
    {
        ...
        for (const _ckKey of Object.keys(_bRes)) {
            if (_ckKey.startsWith('value') && _ckKey.length > 5 && _ckKey.charCodeAt(5) >= 65 && _ckKey.charCodeAt(5) <= 90 && !_ckAllowed.includes(_ckKey) && _bRes[_ckKey] !== undefined) {
                errors.push("The Profile 'http://hl7.org/fhir/uv/ips/StructureDefinition/Observation-pregnancy-status-uv-ips' definition allows for the type Quantity, CodeableConcept, string, boolean, integer, Range, Ratio, SampledData, time, dateTime, Period but found type " + _ckKey.substring(5));
            }
        }
    }
