import { validateObservationPregnancyOutcomeUvIps } from "./ObservationPregnancyOutcomeUvIps.js";
...
export class ObservationPregnancyOutcomeUvIpsClass {
    constructor(resource) {
        this.resource = resource;
        ensureProfile(this.resource, 'http://hl7.org/fhir/uv/ips/StructureDefinition/Observation-pregnancy-outcome-uv-ips');
    }
    /** Validate current resource instance. */
    async validate(options) {
        return validateObservationPregnancyOutcomeUvIps(this.resource, options);
    }
    getResource() {
    ...
    /**
     * Create a minimal randomized instance of ObservationPregnancyOutcomeUvIps.
    ...
    static random(overrides = {}, opts) {
        ...
        const base = { resourceType: 'Observation', id: randomId(), status: "final", code: skeletonCodeableConcept('http://loinc.org', ['11636-8']), subject: { reference: 'Patient/' + randomId() }, effectiveDateTime: randomDate(), };
        // Always include meta.profile with this profile's canonical URL if available & only for resource profiles
        ensureProfile(base, 'http://hl7.org/fhir/uv/ips/StructureDefinition/Observation-pregnancy-outcome-uv-ips');
        // Enrich the base resource with common fields using centralized enrichment logic
        enrichResource(base, 'Observation', 'http://hl7.org/fhir/uv/ips/StructureDefinition/Observation-pregnancy-outcome-uv-ips', ObservationPregnancyOutcomeUvIpsClass._fieldPatterns, ObservationPregnancyOutcomeUvIpsClass._forbiddenFields, ObservationPregnancyOutcomeUvIpsClass.valueSetBindings, ObservationPregnancyOutcomeUvIpsClass._codeResolver, ObservationPregnancyOutcomeUvIpsClass._fieldOrder);
        // Cast through unknown to acknowledge dynamic enrichment
        return { ...base, ...overrides };
    }
    /** Convenience: returns a class instance wrapping a randomized resource. */
    static randomClass(overrides = {}, opts) {
        return new ObservationPregnancyOutcomeUvIpsClass(this.random(overrides, opts));
    }
}
/** Pattern constraints for profile fields (used by random() generator) */
ObservationPregnancyOutcomeUvIpsClass._fieldPatterns = {
...
ObservationPregnancyOutcomeUvIpsClass._forbiddenFields = ["bodySite", "specimen", "device", "referenceRange", "component"];
/** FHIR element ordering for this profile's base resource (used by enrichResource for correct JSON field order) */
ObservationPregnancyOutcomeUvIpsClass._fieldOrder = ["id", "meta", "implicitRules", "language", "text", "contained", "extension", "modifierExtension", "identifier", "basedOn", "partOf", "status", "category", "code", "subject", "focus", "encounter", "effective[x]", "issued", "performer", "value[x]", "dataAbsentReason", "interpretation", "note", "bodySite", "method", "specimen", "device", "referenceRange", "hasMember", "derivedFrom", "component"];
/** ValueSet bindings for coded fields - maps field name to ValueSet URL */
ObservationPregnancyOutcomeUvIpsClass.valueSetBindings = {
    ...
    "code": "http://hl7.org/fhir/uv/ips/ValueSet/pregnancies-summary-uv-ips",
...
ObservationPregnancyOutcomeUvIpsClass._codeResolver = (() => {
