...
|---|---|---|
| `@proxy-smart/auth` | The SMART authorization layer: launch context, session handling, scope gating, token enrichment | [README](https://github.com/proxy-smart/proxy-smart/blob/main/packages/auth/README.md) |
...
`@proxy-smart/elysia-mcp` treats an existing typed route table as the tool catalog it already structurally is. Routes stay the single source of truth, which is why a route added to the admin API is a tool without anyone declaring it twice.
